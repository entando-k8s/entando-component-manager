...
/**
 * This takes control of the environment variables using {@link Mockito#mockStatic}. While there are maps of mock
 * environment variables, the getenv functions will be directed at them. Otherwise, the original ProcessEnvironment
...
public class EnvironmentVariableMocker {
    ...
    /**
     * The equivalent of <code>getenv</code> in the original ProcessEnvironment, assuming that mocking is "turned on".
     *
    ...
    /**
     * On Windows, this returns <code>String</code> and converts the inbound Map. On Linux/Mac this takes a second
     * parameter and returns <code>byte[]</code>. Implementations ripped from the JDK source implementation.
     *
    ...
    /**
     * Attach a map as the mutable replacement environment variables for now. This can be done multiple times and each
     * time the replacement will supersede the maps before. Then when {@link #pop()} is called, we'll rollback to the
     * previous.
     *
     * @param newEnvironmentVariables the mutable map - note: this will be populated by the current environment
     */
    ...
    /**
     * Remove the latest set of mock environment variables. This will run all the way to empty, after which the original
     * implementation of the getenv functions will be called directly again.
     *

...
public class BundleStatusHelper {
    ...
    public BundlesStatusItem composeBundleStatusItemByName(String name,
        ...
        // search into installed but not deployed
        return composeBundleStatusItemByName(name, installedButNotDeployed,
                BundleStatus.INSTALLED_NOT_DEPLOYED, EntandoBundleEntity::getVersion)
                // search into installed and deployed
                .or(() -> composeBundleStatusItemByName(name, installedBundleEntities, BundleStatus.INSTALLED,
                        EntandoBundleEntity::getVersion))
                // search into not installed but deployed
                .or(() -> composeBundleStatusItemByName(name, deployedBundles, BundleStatus.DEPLOYED))
                // otherwise return NOT FOUND
                .orElseGet(() -> new BundlesStatusItem(name, BundleStatus.NOT_FOUND, null));
    }
     ...
     * corresponding to the received params.
     *
     * @param name             the name identifying the bundle to search
     * @param bundleEntityList the list of EntandoBundleEntity in which search the bundle with the url identified by
     *                         repoUrl
     * @param bundleStatus     the status to assign to the BundlesStatusItem to return if the repoUrl if found in the
     *                         list
     ...
     *          in the receive list, an empty one otherwise
     */
    private Optional<BundlesStatusItem> composeBundleStatusItemByName(String name,
            List<EntandoBundleEntity> bundleEntityList, BundleStatus bundleStatus,
            Function<EntandoBundleEntity, String> versionGetFn) {

        return bundleEntityList.stream()
                .filter(bundle -> !ObjectUtils.isEmpty(bundle.getRepoUrl()))
                .filter(bundle -> bundle.getName().equals(name))
                .findFirst()
                .map(bundle -> new BundlesStatusItem(bundle.getRepoUrl(), bundleStatus, versionGetFn.apply(bundle)));
    }
     ...
     *          in the receive list, an empty one otherwise
     */
    private Optional<BundlesStatusItem> composeBundleStatusItemByName(String name, List<EntandoBundle> bundleList,
            BundleStatus bundleStatus) {

        return bundleList.stream()
                .filter(bundle -> !ObjectUtils.isEmpty(bundle.getRepoUrl()))
                .filter(bundle -> bundle.getCode().equals(name))
                .findFirst()
                .map(bundle -> new BundlesStatusItem(bundle.getRepoUrl(), bundleStatus, null));
    }
}
